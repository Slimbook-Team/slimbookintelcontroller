...
try:
    ...
    logger.critical('Cannot open log file /var/slimbookintelcontroller.log, using /tmp/slimbookintelcontroller.log')
...
AUTOSTART_DESKTOP = os.path.join(HOMEDIR, ".config/autostart/slimbookintelcontroller-autostart.desktop")
...
# Read proc patron
patron = re.compile(r'[ ](?P<version>\w\d)[-](?P<number>[0-9]{4,5})(?P<line_suffix>\w*)[ ]')
search_cpu = patron.search(cpu).groupdict()
model_cpu = '{version}-{number}{line_suffix}'.format(**search_cpu)
...
logger.debug(
    ...
    "CPU Numbers: {number} | CPU Letters: {line_suffix}.".format(
        model_cpu=model_cpu, **search_cpu
    )
)
...
class SlimbookINTEL(Gtk.ApplicationWindow):
    ...
    def on_btnAceptar_clicked(self, widget):
        ...
        # Check secureboot
        exit_code, msg = subprocess.getstatusoutput('mokutil --sb-state | grep -i "SecureBoot disabled"')
        ...
        if exit_code:
            ...
            logger.debug("Variable |   mode    | updated, actual value: {} ...".format(self.modo_actual))
            config.set('CONFIGURATION', "autostart", self.autostart_actual)
            logger.debug("Variable | autostart | updated, actual value: {} ...".format(self.modo_actual))
            config.set('CONFIGURATION', "show-icon", self.indicador_actual)
            logger.debug("Variable | show-icon | updated, actual value: {} ...".format(self.modo_actual))
            ...
            self.dialog(_("CPU Warning"),
                        _("Your CPU model is nor supported. To add it by your own, check the tutorial Link / "
    ...
    # CONTENT --------------------------------------------------------------------------------
        ...
        pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(
            filename=os.path.join(CURRENT_PATH, 'images/slimbookintelcontroller_header.png'),
            width=700,
            height=200,
        ...
        if cpu_thermal_zone is not None:
            exit_code, cpu_temp = subprocess.getstatusoutput(
                r"cat /sys/class/thermal/{}/temp | sed 's/\(.\)..$/ °C/'".format(cpu_thermal_zone))
            if exit_code:
    ...
    # RADIOS ---------------------------------------------------------------------------------
        ...
        rbutton1_img = Gtk.Image.new_from_file(os.path.join(CURRENT_PATH, 'images/modo1.png'))
        ...
        rbutton2_img = Gtk.Image.new_from_file(os.path.join(CURRENT_PATH, 'images/modo2.png'))
        ...
        rbutton3_img = Gtk.Image.new_from_file(os.path.join(CURRENT_PATH, 'images/modo3.png'))
    ...
    # BTNABOUT_US ----------------------------------------------------------------------------
        ...
        version_parser.read(os.path.join(CURRENT_PATH, '../slimbookintelcontroller.desktop'))
...
def create_config_file():
    if os.path.isfile(CONFIG_FILE):
        logger.info('File .conf already exists\n')
    else:
...
def setup_config_file():
    ...
    config.set("CONFIGURATION", "mode", "low",)
    config.set("CONFIGURATION", "autostart", "off",)
    config.set("CONFIGURATION", "show-icon", "off",)
    config.set("CONFIGURATION", "cpu", model_cpu)

    if not config.has_section('PROCESSORS'):
        config.add_section('PROCESSORS')
    config.set("PROCESSORS", 'i3-10110U', '10@12/15@25/20@30/ 10/15/25/ w')
    config.set("PROCESSORS", 'i3-1005G1', '10@12/15@25/20@30/ 13/15/25/ w')

    config.set("PROCESSORS", 'i5-8250U', '10@12/15@20/20@35/ 10/15/25/ w')
    config.set("PROCESSORS", 'i5-8265U', '10@12/15@20/20@35/ 10/15/25/ w')
    config.set("PROCESSORS", 'i5-10210U', '10@12/15@20/20@30/ 10/15/25/ w')
    config.set("PROCESSORS", 'i5-1035G1', '10@12/15@20/20@35/ 13/15/25/ w')

    config.set("PROCESSORS", 'i7-7500U', '10@12/15@20/20@35/ 7.5/15/25/ w')
    config.set("PROCESSORS", 'i7-8550U', '10@12/15@25/30@45/ 10/15/25/ w')
    config.set("PROCESSORS", 'i7-8565U', '10@12/15@25/30@45/ 10/15/25/ w')
    config.set("PROCESSORS", 'i7-1065G7', '10@12/15@20/25@45/ 12/15/25/ w')
    config.set("PROCESSORS", 'i7-10510U', '10@12/15@25/30@50/ 10/15/25/ w')
    config.set("PROCESSORS", 'i7-10750H', '15@20/30@45/54@70/ 35/45/54/ w')
    config.set("PROCESSORS", 'i7-1165G7', '10@12/15@25/30@50/ 12/15/28/ w')
