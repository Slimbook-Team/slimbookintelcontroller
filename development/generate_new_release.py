...
APP = "SlimbookIntelController"
CURRENT_PATH = os.path.dirname(os.path.realpath(__file__))
VERSION_FILE = ConfigParser()
VERSION_FILE.read(path.join(CURRENT_PATH, "..", "{}.desktop".format(APP.casefold())))
VERSION = (
    VERSION_FILE.get("Desktop Entry", "Version")
    if VERSION_FILE.has_option("Desktop Entry", "Version")
    else None
)
BUILD_PATH = (
    path.join(CURRENT_PATH, "..", "..", "..", "{}_{}".format(APP, VERSION))
    if VERSION
    else None
)
...
CHILD_BUILD_PATH = (
    path.join(BUILD_PATH, "{}-{}".format(APP, VERSION).casefold()) if VERSION else None
)
...
print(p.wait())
p = subprocess.Popen(
    "debuild -D", shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
)
print(p.wait())
